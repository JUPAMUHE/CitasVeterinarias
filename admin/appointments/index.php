				...
				<tbody>
					...
					?>
						...
						<tr>
							...
							<td align="center">
								<?php 
									$hora = $conn->query("SELECT TIMESTAMPDIFF(HOUR,NOW(),date_sched) AS hora FROM `appointments` WHERE id = '".$row['aid']."'");
									$horaEditar = $hora->fetch_assoc();
									$hora =$horaEditar['hora'];
				                    ...
				                    <a class="dropdown-item view_data" href="javascript:void(0)" data-id="<?php echo $row['aid'] ?>"> Ver</a>
										<?php if($hora>2):?>
											<div class="divider"></div>
												...
												<a class="dropdown-item delete_data" href="javascript:void(0)" data-id=<?php echo $row['aid'];?>>Elimar</a>
											</div>
										<?php endif;?>
									</div> 
							</td>
...
<script>
	...
	$(document).ready(function(){
		...
		$('.delete_data').click(function(){
			uni_modal("Cancelar Cita","appointments/delete.php?id="+$(this).attr('data-id'),'mid-large')
			location.reload();
